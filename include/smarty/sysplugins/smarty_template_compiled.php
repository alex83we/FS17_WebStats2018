...
class Smarty_Template_Compiled extends Smarty_Template_Resource_Base {
	...
	public function populateCompiledFilepath(Smarty_Internal_Template $_template) {
		...
		if ($_template->source->isConfig) {
			$_flag = '_' . (( int ) $_template->smarty->config_read_hidden + ( int ) $_template->smarty->config_booleanize * 2 + ( int ) $_template->smarty->config_overwrite * 4);
		} else {
	...
	public function process(Smarty_Internal_Template $_template) {
		...
		if ($_template->source->handler->recompiled || ! $_template->compiled->exists || $_template->smarty->force_compile || ($_template->smarty->compile_check && $_template->source->getTimeStamp () > $_template->compiled->getTimeStamp ())) {
