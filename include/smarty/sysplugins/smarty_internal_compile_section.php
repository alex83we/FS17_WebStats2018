...
class Smarty_Internal_Compile_Section extends Smarty_Internal_Compile_Private_ForeachSection {
	...
	public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler) {
		...
		$attributes = array (
				'name' => $compiler->getId ( $_attr ['name'] ) 
		);
		unset ( $_attr ['name'] );
		...
		if (! isset ( $propValue ['start'] )) {
			...
			$start_code = array (
					...
					6 => '-1',
					7 => ', ',
					8 => "{$propValue['start']} + {$propValue['loop']}",
					10 => ')',
					11 => ' : ',
					12 => 'min(',
					13 => $propValue ['start'],
			...
			if ($propType ['start'] == 0) {
				...
				if ($propValue ['start'] < 0) {
					...
					if ($propType ['start'] == 0) {
						$start_code = array (
								max ( $propValue ['step'] > 0 ? 0 : - 1, $propValue ['start'] + $propValue ['loop'] ) 
						);
					}
				} else {
					for($i = 1; $i <= 11; $i ++) {
					...
					if ($propType ['start'] == 0) {
						$start_code = array (
								min ( $propValue ['step'] > 0 ? $propValue ['loop'] : $propValue ['loop'] - 1, $propValue ['start'] ) 
						);
					}
				}
			}
		...
		if (! isset ( $_attr ['start'] ) && ! isset ( $_attr ['step'] ) && ! isset ( $_attr ['max'] )) {
			...
			if ($propType ['total'] == 0) {
				$propValue ['total'] = min ( ceil ( ($propValue ['step'] > 0 ? $propValue ['loop'] - $propValue ['start'] : ( int ) $propValue ['start'] + 1) / abs ( $propValue ['step'] ) ), $propValue ['max'] );
			} else {
				$total_code = array (
						1 => 'min(',
						2 => 'ceil(',
...
class Smarty_Internal_Compile_Sectionelse extends Smarty_Internal_CompileBase {
	...
	public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler) {
		...
		list ( $openTag, $nocache, $local, $sectionVar ) = $this->closeTag ( $compiler, array (
				'section' 
		) );
...
class Smarty_Internal_Compile_Sectionclose extends Smarty_Internal_CompileBase {
	...
	public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler) {
		...
		list ( $openTag, $compiler->nocache, $local, $sectionVar ) = $this->closeTag ( $compiler, array (
				'section',
				'sectionelse' 
		) );
