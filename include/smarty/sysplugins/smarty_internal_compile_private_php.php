...
class Smarty_Internal_Compile_Private_Php extends Smarty_Internal_CompileBase {
	...
	public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler, $parameter) {
		...
		if ($_attr ['type'] == 'xml') {
			...
			$compiler->parser->current_buffer->append_subtree ( $compiler->parser, new Smarty_Internal_ParseTree_Tag ( $compiler->parser, $compiler->processNocacheCode ( "<?php echo '" . $output . "';?>", $compiler, true ) ) );
		...
		if ($_attr ['type'] != 'tag') {
			if ($compiler->php_handling == Smarty::PHP_REMOVE) {
				...
				$output = preg_replace_callback ( '#(<\?(?:php|=)?)|(<%)|(<script\s+language\s*=\s*["\']?\s*php\s*["\']?\s*>)|(\?>)|(%>)|(<\/script>)#i', array (
						$this,
						'quote' 
				), $_attr ['code'] );
