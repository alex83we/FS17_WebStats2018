...
abstract class Smarty_Internal_TemplateCompilerBase {
	...
	public function compileTemplateSource(Smarty_Internal_Template $template, $nocache = null, Smarty_Internal_TemplateCompilerBase $parent_compiler = null) {
		try {
			...
			// add file dependency
			$this->parent_compiler->template->compiled->file_dependency [$this->template->source->uid] = array (
	...
	public function processNocacheCode($content, $is_code) {
		...
		if ($is_code && ! empty ( $content )) {
			...
			if ((! ($this->template->source->handler->recompiled) || $this->forceNocache) && $this->template->caching && ! $this->suppressNocacheProcessing && ($this->nocache || $this->tag_nocache)) {
	...
	public function trigger_template_error($args = null, $line = null, $tagline = null) {
		...
		if (in_array ( $this->template->source->type, array (
				'eval',
				'string' 
		) )) {
